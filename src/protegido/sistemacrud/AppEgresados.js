...
import AppFormEgresados from './AppFormEgresados'
...
const AppEgresados = (props) => {
  ////// Lectura fnRead ///////////
  const [docBD, setDocBD] = useState([]);
  //console.log(docBD);
  const fnRead = () => {
    const xColeccionConQuery = query(collection(db, 'egresados'));
    //const xColeccionConQuery = query(collection(db, "egresados"), where("nombre", "!=", ""));
  ...
  const fnDelete = async (xId) => {
    if(window.confirm("Confirme para eliminar")){
      await deleteDoc(doc(db, 'egresados', xId));
      ...
      <div className='card bs-secondary p-3 mt-3'> 
        ...
        <div className='col-md-12 p-2'>
          <div className='card mb-1'>
            <AppFormEgresados {...{idActual, setIdActual}} />
          </div>
        </div>

        <div className='col-md-12 p-2'>
          <div className='card mb-1'>
            <h2>Lista de Egresados(AppEgresados.js)</h2>
          </div>
        </div>
